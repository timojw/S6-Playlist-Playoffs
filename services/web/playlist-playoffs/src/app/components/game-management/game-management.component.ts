...
export class GameManagementComponent implements OnInit {
  ...
  fetchGameDetails(): void {
    this.http.get(`http://localhost:8080/api/game/${this.gameId}`).subscribe(
      (data: any) => {
        this.game = data;
  ...
  saveVotes(): void {
    ...
    this.http.post('http://localhost:8080/api/votes', { votes }, { headers }).subscribe(
      () => {
  ...
  loadVotes(): void {
    ...
    this.http.get<VotesResponse>(`http://localhost:8080/api/votes/${this.gameId}`).subscribe(
      (response: any) => {
